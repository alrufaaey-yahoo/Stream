...
def ytdl_extract_with_fallback(
    query: str, video: bool = True
) -> Tuple[Optional[Dict], bool]:
    opts_with_cookies = YTDL_OPTS_WITH_COOKIES.copy()
    opts_with_cookies["format"] = (
        "bestaudio/best" if not video else "bestvideo+bestaudio/best"
    )
...
async def start_next_in_queue(chat_id: int) -> None:
    ...
    threading.Thread(
        target=run_ffmpeg,
        args=(chat_id, cmd, item.get("input_file"), stream_data),
        daemon=True,
    ).start()
...
async def play(_, m: Message):
    ...
    try:
        suffix = (
            m.reply_to_message.file.file_name.split(".")[-1]
            if hasattr(m.reply_to_message, "file") and m.reply_to_message.file
            else "tmp"
        )
...
async def uplay(_, m: Message):
    ...
    enqueue_rt(item)
    await send_log(
        m.from_user.id, m.from_user.username or "unknown", m.chat.id, "UPLAY"
    )
